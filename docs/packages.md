...
## Deterministic Lockfiles (`flake.lock`)

Flake ensures reproducible builds across teams and machines via `flake.lock`.

### Generating and Checking Lockfiles
```bash
# Generate or refresh flake.lock:
flake lock

# Verify that flake.lock matches flake.toml without modifying it (e.g. in CI):
flake lock --check

# Re-resolve dependency paths and recalculate checksums:
flake update
```

### The Lockfile Format
```toml
# Auto-generated by Flake. Do not edit manually.
lockfile_version = 1
root_package = "app"
...
name = "core_lib"
version = "0.1.0"
source = "path+../core_lib"
checksum = "94f1ac17db3672a1"
dependencies = []
...
version = "0.6.0"
source = "root"
checksum = "182f09312048997a"
dependencies = ["core_lib"]
```

When present in a package root, `flake run`, `flake build`, and `flake check` automatically verify that the locked package graph matches the declared dependencies.
